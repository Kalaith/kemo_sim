...
export function calculateGameStats(
  ...
  const highestStatKemonomimi = kemonomimi.reduce((highest, kemono) => {
    const currentTotal = Object.values(kemono.stats).reduce((sum, stat) => sum + stat, 0);
    const highestTotal = highest ? Object.values(highest.stats).reduce((sum, stat) => sum + stat, 0) : 0;
    return currentTotal > highestTotal ? kemono : highest;
  }, null as Kemonomimi | null);

  const mostTrainedKemonomimi = kemonomimi.reduce((mostTrained, kemono) => {
  ...
  const oldestKemonomimi = kemonomimi.reduce((oldest, kemono) => {
